...
export default function Hyperspeed({ effectOptions = DEFAULT_EFFECT_OPTIONS, className = "" }) {
  ...
  useEffect(() => {
    ...
    class App {
      constructor(appContainer, options = {}) {
        ...
        this.renderer.setSize(appContainer.offsetWidth, appContainer.offsetHeight, false);
        this.renderer.setPixelRatio(window.devicePixelRatio);
        this.composer = new EffectComposer(this.renderer);
        appContainer.append(this.renderer.domElement);

        this.camera = new THREE.PerspectiveCamera(
          options.fov,
          appContainer.offsetWidth / appContainer.offsetHeight,
          0.1,
          10000
        );
  ...
  return <div ref={containerRef} className={`hyperspeed ${className}`.trim()} />;
}
